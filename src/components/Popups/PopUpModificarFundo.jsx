...
import { insertarPlaga, modificarFundo } from '../../services/adminService';

function PopUpModificarFundo({show, setShow, row}) {
    const [showSection, setShowSection] = React.useState(false);
    const [snackbar, setSnackbar] = React.useState(null);

    const [nombre, setNombre] = React.useState(row.nombreFundo);
    const [descripcion, setDescripcion] = React.useState(row.descripcion);
    const [latitud, setLatitud] = React.useState(row.latitud);
    const [longitud, setLongitud] = React.useState(row.longitud);
    ...
    const addNewFundo = async(data) => {
      const result = await modificarFundo(data);
      if (result.status == 200) {
        ...
        alert('Algo salio mal al editar el dato');
      }
      limpiarDatos();
      setShow(false);
    };
...
export default PopUpModificarFundo